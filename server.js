...
async function createPayPalOrder(totalPrice, description) {
    ...
    return_url: `https://e-bike-backend-hl91.onrender.com`, // orderId ще не існує - замовлення створиться тільки після оплати
...
app.post('/api/auth/register', (req, res) => {
    try {
        ...
        db.get('SELECT * FROM users WHERE phone = ?', [phone], (err, user) => {
            ...
            // Зберігаємо користувача (у реальному застосунку треба хешувати пароль!)
            db.run(
